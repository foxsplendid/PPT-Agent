#!/usr/bin/env python3
"""
Paper PPT Agent - LaTeX Formula Renderer

Render Strategist-declared LaTeX formulas to transparent PNG assets.
The script reads an explicit manifest; it never scans design specs or source
content for dollar-delimited math.

Run from the generation workspace root with the workspace Python
(`agent_task.json.paths.python` or `PAPER_PPT_PYTHON`):
    "<python>" skills/paper-ppt-generate/scripts/latex_render.py .
...
Examples:
    "<python>" skills/paper-ppt-generate/scripts/latex_render.py .
    "<python>" skills/paper-ppt-generate/scripts/latex_render.py . --providers codecogs,quicklatex,mathpad,wikimedia
...
def _render_codecogs(
    ...
    req = urllib.request.Request(url, headers={"User-Agent": "Paper-PPT-Agent/1.0"})
...
def _render_wikimedia(
    ...
    check_req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "Paper-PPT-Agent/1.0",
        },
        method="POST",
    )
    try:
    ...
    render_req = urllib.request.Request(
        ...
        headers={"User-Agent": "Paper-PPT-Agent/1.0"},
    )
